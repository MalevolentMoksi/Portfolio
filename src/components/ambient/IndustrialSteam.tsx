...
function pickPipePositions(count: any) {
  if (count === 0) return [];
  const positions: any = [];
  let attempts = 0;
  while (positions.length < count && attempts < 300) {
    ...
    if (positions.every((p: any) => Math.abs(p - x) >= MIN_PIPE_GAP)) {
      positions.push(x);
    }
  }
  return positions.sort((a: any, b: any) => a - b);
}

/* ─── SVG d'un tuyau industriel ─── */
const PipeFixture = ({ x }: any) => (
  <div className="industrial-pipe-fixture" style={{ left: `${x}%` }} aria-hidden="true">
...
const IndustrialSteam = () => {
  ...
  const spawnPuff = useCallback((ventX: any) => {
    const id = ++_puffId;
