...
const edgeRepulsion = (bx, by, ex, ey, nx, ny) => {
...
const AmbientBoids = () => {
  ...
  useEffect(() => {
    ...
    const tick = (ts) => {
      ...
      for (let i = 0; i < boids.length; i++) {
        ...
        if (sepN > 0) { ax += (sepX / sepN) * W_SEPARATION; ay += (sepY / sepN) * W_SEPARATION; }
        if (aliN > 0) { ax += (aliVx / aliN - bi.vx) * W_ALIGNMENT; ay += (aliVy / aliN - bi.vy) * W_ALIGNMENT; }
        if (cohN > 0) { ax += (cohX / cohN - bi.x) * W_COHESION * 0.01; ay += (cohY / cohN - bi.y) * W_COHESION * 0.01; }
        ...
        if (mr) {
          // Bord gauche
          if (bi.x > mr.left - MAIN_AVOID_R && bi.x < mr.left && bi.y > mr.top && bi.y < mr.bottom) {
            const [fx, fy] = edgeRepulsion(bi.x, bi.y, mr.left, bi.y, -1, 0);
            ax += fx; ay += fy;
          }
          // Bord droit
          if (bi.x > mr.right && bi.x < mr.right + MAIN_AVOID_R && bi.y > mr.top && bi.y < mr.bottom) {
          ...
          // Bord bas
          if (bi.y > mr.bottom && bi.y < mr.bottom + MAIN_AVOID_R && bi.x > mr.left && bi.x < mr.right) {
  ...
  return createPortal(
    <canvas
      ref={canvasRef}
      className="ambient-boids-canvas"
      aria-hidden="true"
    />,
