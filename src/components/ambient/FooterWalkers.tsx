...
const ExplorerSVG = () => (
  <svg viewBox="0 0 22 28" width="22" height="28" fill="none" aria-hidden="true">
    ...
    <path
      d="M8 9 Q6 12, 5 20 Q5.5 21, 7 20"
      stroke="currentColor"
      strokeWidth="0.7"
      opacity="0.5"
      fill="none"
    ...
    <g className="walker-head">
      <path
        d="M8 10 Q8 4, 10.5 3 Q13 4, 13 8 L13 10"
        stroke="currentColor"
        strokeWidth="1"
        fill="none"
      />
...
const CampfireSVG = () => (
  ...
  >
    ...
    <path
      className="campfire-flame campfire-flame--1"
    ...
    />
    <path
      className="campfire-flame campfire-flame--2"
    ...
    />
    <path
      className="campfire-flame campfire-flame--3"
    ...
    <circle
      className="campfire-spark campfire-spark--1"
      cx="9"
      cy="5"
      r="0.5"
      fill="currentColor"
    ...
    />
    <circle
      className="campfire-spark campfire-spark--2"
      cx="11"
      cy="3.5"
      r="0.4"
      fill="currentColor"
    ...
    />
    <circle
      className="campfire-spark campfire-spark--3"
      cx="10"
      cy="2"
      r="0.3"
      fill="currentColor"
...
const SingleWalker = ({ id: walkerId, character, startX, phases, footerRef, onDone }: any) => {
  ...
  const CharSVG = CHARACTERS[character as keyof typeof CHARACTERS] || GuardianSVG;
  ...
  const runPhase = useCallback(() => {
    ...
    switch (phase.action) {
      case 'enter': {
        ...
        const nextWalk = phases.find(
          (p: any, i: any) => i > phaseIdxRef.current && p.action === 'walkTo'
        );
  ...
  return (
    <div
      ref={elRef}
      className={['footer-walker', `walker--${phaseAction}`, visible ? 'walker--visible' : ''].join(
        ' '
      )}
...
const FooterWalkers = () => {
  ...
  const handleWalkerDone = useCallback((walkerId: any) => {
    if (!mountedRef.current) return;
    setWalkers((prev) => prev.filter((w) => w.id !== walkerId));
  }, []);

  /** Lance un script (ou type forcé via console) */
  const spawnScript = useCallback((scriptId?: string) => {
  ...
  // Console APIs
  useEffect(() => {
    window.spawnWalker = (scriptId?: string) => spawnScript(scriptId);
    window.getWalkerState = () => walkers;
    return () => {
      delete window.spawnWalker;
      delete window.getWalkerState;
    };
  }, [spawnScript, walkers]);
...
function estimateScriptDuration(phases: any) {
  let ms = 0;
  let prevX = null;
