...
const ElectricalGrid = () => {
  ...
  return createPortal(
    <div
      ...
      aria-hidden="true"
    >
      <svg className="industrial-grid-svg" viewBox="0 0 100 100" preserveAspectRatio="none">
      ...
      {activeArcs && (
        <svg className="industrial-grid-arcs" viewBox="0 0 100 100" preserveAspectRatio="none">
          {ARC_PATHS.map((path, index) => (
            <path key={path} d={path} className="industrial-grid-arc" style={arcStyleSeed[index]} />
          ))}
        </svg>
      )}
    </div>,
