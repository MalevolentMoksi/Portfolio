...
const CatchGame = ({ botPosRef, onBotCatch, onGameEnd, ballInfoRef, stats }: any) => {
  ...
  const [bestScore, setBestScore] = useState(() => parseInt(safeLocalGet(LS_BEST) || '0', 10));
  ...
  const buildAim = (fromX: any, fromY: any, vx: any, vy: any) => {
      ...
      if (holder === 'player') {
        ...
        if (dvx * dvx + dvy * dvy > AIM_DIRTY_THRESH * AIM_DIRTY_THRESH) {
          ...
          if (aimPathRef.current)
            aimPathRef.current.setAttribute('d', buildAim(bp.x, bp.y, sv.x, sv.y));
        }
      ...
      if (holderRef.current === 'flying') {
        ...
        if (hitDist < BOT_CATCH) {
          ...
          botHoldCounterRef.current =
            CATCH_BOT_HOLD_MIN + Math.floor(Math.random() * CATCH_BOT_HOLD_RANGE);
          setRallies((r) => r + 1);
