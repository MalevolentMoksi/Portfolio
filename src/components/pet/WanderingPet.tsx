...
interface WanderingPetProps {
  ...
  mouthExpr: string;
  petMood: string;
  onInteract: (action: 'feed' | 'pet' | 'play' | 'catch') => void;
  onBehavior: (reaction: string) => void;
  onBotCatchSuccess: () => void;
  onThought: (thought: string | ThoughtItem) => void;
  onHoverPet: () => void;
  cooldowns: { feed: number; pet: number; play: number };
  thoughtQueue: ThoughtItem[];
  hudThought: string;
  sizeScale: number;
  speedMult: number;
  isSleeping: boolean;
  moodSpinActive: boolean;
  petName: string;
  onRename: (newName: string) => void;
  feedIconIndex: number;
  achievements: string[];
...
const WanderingPet = forwardRef<unknown, WanderingPetProps>(function WanderingPet(
  {
    stats,
    expression,
    eyeState,
  ...
  const velRef = useRef({
    x: (Math.random() - 0.5) * BASE_SPEED * 2,
    y: (Math.random() - 0.5) * BASE_SPEED * 2,
  });
  ...
  const attractTargetRef = useRef<any>(null);
  ...
  const ballInfoRef = useRef<any>(null);
  // Cached intercept target — recomputed every 10 frames, not every frame
  const cachedInterceptRef = useRef<any>(null);
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      scrollHistoryRef.current = scrollHistoryRef.current.filter(
        (e) => now - e.time < SCROLL_DIZZY_WINDOW
      );
      ...
      scrollIdleTimerRef.current = setTimeout(() => {
        ...
        for (const el of [footerEl, mainEl]) {
          ...
          if (rect.top > 60 && rect.top < vh - 80) {
            const rx =
              HALF +
              Math.random() * Math.max(0, Math.min(rect.width, window.innerWidth) - PET_SIZE);
  ...
  useEffect(() => {
    const tick = (now: any) => {
      ...
      // ── Delta-time calculation (frame-rate independent physics) ──
      // dt == 1.0 at 60 fps, ~0.42 at 144 fps, capped at 3× to absorb tab-resume spikes
      const dt =
      ...
      if (tickNow - lastTickTimeRef.current > 500) {
        ...
        prevTimeRef.current = null; // Reset dt calculation on resume
      }
      ...
      if (attractTargetRef.current) {
        ...
        p.y = clamp(p.y + v.y * dt, PET_TOP_MIN, vh - HALF);
        ...
        if (frameRef.current % 2 === 0) {
          ...
          if (v.x > 0.08) flipHysteresisRef.current = Math.min(flipHysteresisRef.current + 1, 20);
            ...
            flipHysteresisRef.current = Math.max(flipHysteresisRef.current - 1, -20);
      ...
      if (isRestingRef.current && restTargetRef.current) {
        ...
        // Update position with wall clamp (dt-scaled)
        p.x = clamp(p.x + v.x * dt, HALF, vw - HALF);
        p.y = clamp(p.y + v.y * dt, PET_TOP_MIN, vh - HALF);
        if (frameRef.current % 2 === 0) {
          petLeft.set(p.x - HALF);
      ...
      if (sleeping && !attractTargetRef.current) {
        ...
        v.x += (desiredVRef.current.x - v.x) * 0.01 * dt;
        v.y += (desiredVRef.current.y - v.y) * 0.01 * dt;
      ...
      const dv = desiredVRef.current;
      v.x += (dv.x - v.x) * 0.035 * dt;
      v.y += (dv.y - v.y) * 0.035 * dt;
      // Natural friction (dt-scaled: exponential decay at frame-rate independent speed)
      v.x *= Math.pow(0.984, dt);
      v.y *= Math.pow(0.984, dt);

      let speedCap = MAX_SPEED;

      // ── Gravity effect: pull pet downward when active (dt-scaled) ──
      ...
      if (prox.bounceCooldown > 0) prox.bounceCooldown -= dt;

      // Smooth edge repulsion (softer push when not bouncing hard) (dt-scaled)
      const margin = 40;
      if (!bounced) {
        if (p.x < margin + HALF)
          v.x += 0.18 * Math.pow(1 - Math.max(0, (p.x - HALF) / margin), 1.5) * dt;
        if (p.x > vw - margin - HALF)
          v.x -= 0.18 * Math.pow(1 - Math.max(0, (vw - HALF - p.x) / margin), 1.5) * dt;
        if (p.y < PET_TOP_MIN + 10) v.y += 0.28 * dt;
        if (!gravityActiveRef.current && p.y > vh - margin - HALF)
          v.y -= 0.18 * Math.pow(1 - Math.max(0, (vh - HALF - p.y) / margin), 1.5) * dt;
      }
      ...
      if (!isAttracting && dist > 5) {
        if (petMoodRef.current !== 'sad' && dist < MAGNET_RADIUS) {
          // Happy/content: gently attracted to cursor (dt-scaled)
          const strength = ((MAGNET_RADIUS - dist) / MAGNET_RADIUS) * 0.06;
          v.x += (dx / dist) * strength * dt;
          v.y += (dy / dist) * strength * dt;
          speedCap = MAGNET_SPEED;
        } else if (petMoodRef.current === 'sad' && dist < 120) {
          // Sad: softly avoids the cursor (dt-scaled)
          const pushback = ((120 - dist) / 120) * 0.04;
          v.x -= (dx / dist) * pushback * dt;
          v.y -= (dy / dist) * pushback * dt;
          // Track dwell time near cursor while avoiding — trigger angry thought bubble
          proximityRef.current.avoidDwellFrames += dt;
          if (
            proximityRef.current.avoidDwellFrames >= 30 &&
            proximityRef.current.avoidThoughtCooldown === 0
          ) {
            onThought('exclaim');
      ...
      if (!isAttracting && proximityRef.current.avoidThoughtCooldown > 0)
        proximityRef.current.avoidThoughtCooldown -= dt;
      ...
      // Mettre à jour la position (with bounce already handled above, apply dt-scaled velocity)
      if (!bounced) {
        p.x = clamp(p.x + v.x * dt, HALF, vw - HALF);
        p.y = clamp(p.y + v.y * dt, PET_TOP_MIN, vh - HALF);
      }
      ...
      if (frameRef.current % 2 === 0) {
        ...
        if (prox.exciteCooldown > 0) prox.exciteCooldown -= dt;
        if (prox.speedCooldown > 0) prox.speedCooldown -= dt;
        ...
        if (resumeGraceRef.current === 0) {
          ...
          if (!isAttracting && !isSad && dist < 130 && dist > 8) {
            ...
            prox.dwellFrames = Math.max(0, prox.dwellFrames - 2 * dt);
          }
          ...
          if (
            !isAttracting &&
            !isSad &&
            spd > effectiveMaxSpeed * 0.82 &&
            prox.speedCooldown === 0 &&
            prox.exciteCooldown === 0
          ) {
            onBehavior('excited');
    ...
    const onVisibility = () => {
      if (document.hidden) {
        ...
        lastTickTimeRef.current = Date.now(); // réinitialises le gap détecteur
        prevTimeRef.current = null; // Reset delta-time on visibility change
        rafRef.current = requestAnimationFrame(tick);
      }
    };
    document.addEventListener('visibilitychange', onVisibility);
  ...
  useEffect(() => {
    ...
    const handle = (e: any) => {
      if (
        hudRef.current &&
        !hudRef.current.contains(e.target) &&
        robotRef.current &&
        !robotRef.current.contains(e.target)
      ) {
        setHudOpen(false);
      }
    };
  ...
  const [hudPos, setHudPos] = useState<any>(null);
  useEffect(() => {
    if (!hudOpen) {
      setHudPos(null);
      return;
  ...
  const handleHudKeyDown = useCallback((e: any) => {
    if (e.key === 'Escape') {
      setHudOpen(false);
      return;
    }
    ...
    const focusable = [
      ...hudRef.current.querySelectorAll(
  ...
  useEffect(() => {
    let gravityTimer: any = null;
    window.petGravity = (duration: any) => {
  ...
  useEffect(() => {
    let attractTimer: any = null;
    window.petAttract = (x: any, y: any, duration: any) => {
  ...
  return createPortal(
    <>
      ...
      <motion.div
        ...
        onClick={() => {
          if (!dragHasMovedRef.current) toggleHud();
        }}
            ...
            <div className={`pet-svg-wrap${catchSpring ? ' pet-svg-wrap--catch-spring' : ''}`}>
              ...
              >
                <RobotFace
                  ...
                  eyeState={isResting ? 'cozy' : eyeState}
                  mouthExpr={isResting ? 'cozy' : mouthExpr}
                  gazeX={facingLeft ? -gaze.x : gaze.x}
                  gazeY={gaze.y}
                />
              </motion.div>
            </div>
            ...
            <span className="pet-hud-title">
              ...
              {isEditingName ? (
                <input
                  ...
                  value={nameDraft}
                  onChange={(e: any) => setNameDraft(e.target.value.slice(0, 18))}
                  onBlur={() => {
                    onRename(nameDraft);
              ...
              <Tooltip text={t('common.petHUD.buttons.achievements')}>
                ...
                >
                  ...
                  >
                    ...
                    {/* Anse droite */}
                    <path
                      d="M12 3 Q15 3 15 5.5 Q15 8 12 7.5"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="1.2"
                      strokeLinecap="round"
              ...
              >
                {petMood === 'happy' && (
                  <>
                    ...
                    <circle cx="5.8" cy="7" r="0.8" fill="currentColor" stroke="none" />
                    <circle cx="10.2" cy="7" r="0.8" fill="currentColor" stroke="none" />
                  </>
                )}
                {petMood === 'content' && (
                  <>
          ...
          {stats.hunger < 30 && (
            <p className="pet-hud-needs">
              <svg
                ...
                strokeLinecap="round"
                aria-hidden="true"
                className="pet-hud-hint-icon"
              >
                <path d="M8 1 L14.5 13 L1.5 13 Z" />
            ...
            <div className="pet-stat">
              ...
              <div className="pet-stat-track">
                <motion.div
                  ...
                  animate={{ scaleX: stats.hunger / 100 }}
            ...
            <div className="pet-stat">
              ...
              <div className="pet-stat-track">
                <motion.div
                  ...
                  animate={{ scaleX: stats.happiness / 100 }}
                  transition={{ type: 'spring', stiffness: 80, damping: 18 }}
                />
              </div>
            ...
            {[
              ...
              },
              {
                ...
                icon: (
                  ...
                  >
                    <path
                      d="M3 7.5 Q3 5 5.5 4 Q7 3.5 8 5 Q9 3.5 10.5 4 Q13 5 13 7.5 Q13 11 8 13.5 Q3 11 3 7.5Z"
                      opacity="0.9"
                    />
                  </svg>
                ),
              ...
              const remaining =
                noCooldown || cooldownKey === null
                  ? 0
                  : Math.max(0, (cooldowns[cooldownKey] || 0) - Date.now());
