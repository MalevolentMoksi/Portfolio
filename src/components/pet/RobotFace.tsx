...
import { PET_SIZE } from './petConstants';
...
const RobotFace = ({ expression, eyeState, mouthExpr, gazeX = 0, gazeY = 0 }: RobotFaceProps) => {
        ...
        return (
          <>
            ...
            <motion.circle
              ...
              className="pet-eye-pupil"
              animate={blinkAnim}
              transition={{ ...blinkTrans, delay: 0.08 }}
              style={{ transformBox: 'fill-box', transformOrigin: '50% 50%' }}
            />
      ...
      case 'sleep':
        ...
        return (
          <>
            {/* Left U eye */}
            <path
              d="M14 17 Q14 22 17 22 Q20 22 20 17"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
            ...
            {/* Right U eye */}
            <path
              d="M28 17 Q28 22 31 22 Q34 22 34 17"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.8"
              strokeLinecap="round"
  ...
  const mouthPathMap = MOUTH_PATHS as Record<string, string>;
  ...
  const renderMouth = () => (
    <g className={expr === 'sleep' ? 'pet-mouth-wrap' : undefined}>
      <motion.path
        ...
        animate={{ d: mouthPathMap[me] ?? MOUTH_PATHS.default }}
  ...
  const antennaPathMap = ANTENNA_PATHS as Record<string, { left: string; right: string }>;
  const antPath = antennaPathMap[expr] || ANTENNA_PATHS.content;
