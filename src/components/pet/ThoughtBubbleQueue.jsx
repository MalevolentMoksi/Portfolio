...
const REACTION_ICONS = {
  feed:    (
    <path d="M4 8 Q4 3 8 3 Q12 3 12 8 L12 10 Q12 13 10 13 L10 15 L6 15 L6 13 Q4 13 4 10 Z"
          fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" />
  ),
  petted:  (
    <>
      <path d="M3 9 Q3 6 5.5 5 Q7 4.5 8 6 Q9 4.5 10.5 5 Q13 6 13 9 Q13 12 8 14 Q3 12 3 9Z"
            fill="currentColor" opacity="0.85" />
    </>
  ),
  play:    (
    <>
      <rect x="2" y="5" width="12" height="9" rx="2.5" fill="none" stroke="currentColor" strokeWidth="1.4" />
      <circle cx="5.5" cy="9.5" r="1.4" fill="currentColor" />
      <circle cx="10.5" cy="9.5" r="1.4" fill="currentColor" />
    </>
  ),
  excited: (
    <path d="M8 2 L9.5 6 L14 6 L10.5 9 L12 13 L8 10.5 L4 13 L5.5 9 L2 6 L6.5 6 Z"
          fill="currentColor" />
  ),
};

/* ── Normalise un thought payload ── */
export const normalizeThought = (input) => {
...
const Bubble = ({ thought, idx, petX, petY }) => {
  ...
  /* ── Symbol bubble ── */
  if (type === 'symbol') {
    const sym = THOUGHT_SYMBOLS[content];
    if (!sym) return null;
    return createPortal(
      <div
        ...
        aria-hidden="true"
      >
        <svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor" stroke="none" overflow="visible">
          {sym}
        </svg>
      </div>,
      document.body,
  ...
  if (type === 'reaction') {
    const icon = REACTION_ICONS[content];
    return createPortal(
      <div
...
const ThoughtBubbleQueue = ({ queue, petX, petY }) => {
  if (!queue || queue.length === 0) return null;
  return queue.slice(0, 3).map((thought, idx) => (
