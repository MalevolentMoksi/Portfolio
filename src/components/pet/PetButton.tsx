...
import { MOOD_TEXT_POOL, FACE_COMBOS, THOUGHT_POOLS, FOOD_ICONS } from './petData';
...
const PetButton = () => {
  ...
  const moodTextPool = useMemo<MoodTextPool>(() => {
    const language = i18n.resolvedLanguage ?? 'fr';
    const fromStore = i18n.getResource(language, 'translation', 'common.pet.moodTextPool');
    if (fromStore && typeof fromStore === 'object' && !Array.isArray(fromStore)) {
      return fromStore as MoodTextPool;
    }
    return MOOD_TEXT_POOL as MoodTextPool;
  ...
  const [faceCombo, setFaceCombo] = useState(() => {
    const combos = FACE_COMBOS as Record<string, FaceCombo[]>;
    return pickRandom(combos[getMood(50, 50)] ?? combos.content);
  });
  // Floating thought bubble queue — each entry: { id, type, content, label?, duration }
  const [thoughtQueue, setThoughtQueue] = useState<Array<ThoughtItem & { id: number }>>([]);
  const thoughtQueueRef = useRef<Array<ThoughtItem & { id: number }>>([]);
  const thoughtTimersRef = useRef<Record<number, ReturnType<typeof setTimeout>>>({});
  ...
  const idleReactionRef = useRef<any>(null);
  ...
  const sadSinceRef = useRef<any>(null);
  ...
  const sleepCheckRef = useRef<any>(null);
  ...
  useEffect(() => {
    localSet(LS.feedIndex, String(feedIconIndex));
  }, [feedIconIndex]);
  useEffect(() => {
    localSetJSON(LS.achievements, achievements);
  }, [achievements]);
  useEffect(() => {
  ...
  const triggerReaction = useCallback(
    (r: string) => {
      ...
      setReaction(r);
      const combos = FACE_COMBOS as Record<string, FaceCombo[]>;
      setFaceCombo(pickRandom(combos[r] ?? combos.content));
  ...
  useEffect(() => {
    ...
    const steps: Array<{ delay: number; thought: ThoughtItem }> = [
      {
        delay: 1200,
        thought: { type: 'text', content: t('common.pet.onboarding.0'), duration: 5000 },
      },
      {
        delay: 6300,
        thought: { type: 'text', content: t('common.pet.onboarding.1'), duration: 5000 },
      },
      {
        delay: 11400,
        thought: { type: 'text', content: t('common.pet.onboarding.2'), duration: 5000 },
      },
      {
        delay: 16500,
        thought: { type: 'text', content: t('common.pet.onboarding.3'), duration: 5000 },
      },
    ];
  ...
  useEffect(() => {
    idleReactionRef.current = reaction;
  }, [reaction]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (reaction === null) {
      const combos = FACE_COMBOS as Record<string, FaceCombo[]>;
      setFaceCombo(pickRandom(combos[petMood] ?? combos.content));
      ...
      idleTimerRef.current = setTimeout(
        () => {
          ...
          if (idleReactionRef.current === null && !isSleepingRef.current) {
            ...
            if (Math.random() < 0.4) {
              const thoughtPools = THOUGHT_POOLS as Record<string, string[]>;
              const pool = thoughtPools[chosenReaction] ?? thoughtPools[mood] ?? ['dots'];
        ...
        13000 + Math.random() * 20000
      ); // 13–33 s between idle pulses
    };
    schedule();
    return () => {
  ...
  useEffect(() => {
    ...
    window.getPetStats = () => {
      ...
      return {
        ...
        get mood() {
          return getMood(this.hunger, this.happiness);
        },
      };
    };
    return () => {
      delete window.petReact;
      delete window.getPetStats;
    };
  }, [triggerReaction]);
  ...
  const handleInteract = useCallback(
    (action: InteractionAction) => {
      ...
      if (action !== 'feed' && action !== 'pet' && action !== 'play') {
        return;
      }
      ...
      switch (action) {
        ...
        case 'play':
          setStats((s) => ({
            hunger: clamp(Math.round(s.hunger + 10)),
            happiness: clamp(Math.round(s.happiness + 10)),
          }));
          triggerReaction('play');
          break;
      }
  ...
  const handleRename = useCallback(
    (newName: string) => {
      ...
      if (trimmed !== t('common.pet.defaultName')) {
        ...
        showToast(t('common.pet.toast.renamed', { name: trimmed }), {
          type: 'success',
          duration: 2500,
        });
      }
  ...
  return (
    <>
      ...
      <Tooltip
        text={isSpawned ? t('common.pet.tooltip.recall') : t('common.pet.tooltip.summon')}
        position="bottom"
      >
        <button
          ...
          <span className="pet-toggle-track">
            <span className="pet-toggle-thumb">
              ...
              >
                ...
                <circle
                  className="pet-icon-antenna-tip"
                  cx="12"
                  cy="2"
                  r="1.5"
                  fill="currentColor"
