...
/* ── Helpers ─────────────────────────────────────────── */
const randCell = (snake: Cell[]): Cell => {
  let c: Cell;
  do {
...
const hsColor = (i: number, len: number): string => {
...
const SnakeGame = ({ onClose }: SnakeGameProps) => {
  ...
  const step = (): void => {
    ...
    if (g.dirQueue.length > 0) {
      const queuedDir = g.dirQueue.shift();
      if (queuedDir) g.dir = queuedDir;
    }
  ...
  const startGame = () => {
    ...
    const tick = (): void => {
      ...
      if (count > 0) {
        ...
        if (gRef.current) gRef.current.status = 'playing';
